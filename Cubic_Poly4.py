#Exercise 9.2: Make polynomial subclasses of parabolas 

class Line:
	def __init__(self, c0, c1):
		self.c0 = c0
		self.c1 = c1
	def __call__(self, x):
		print 'Line'
		return self.c0 + self.c1*x
	def table(self, L, R, n):
		#Return a table with n points for L <= x <= R.
		s = ''
		import numpy as np
		for x in np.linspace(L, R, n):
			y = self(x)
			s += '%12g %12g\n' % (x, y)
		return s

class Parabola(Line): #inheriting from Line
	def __init__(self, c0, c1, c2):
		Line.__init__(self, c0, c1)
		self.c2 = c2 #adding new element
	def __call__(self, x):
		print 'Parabola'
		return Line.__call__(self, x) + self.c2*x**2 #adding to equation

class Cubic(Parabola): #inheriting from Parabola
	def __init__(self, c0, c1, c2, c3):
		Parabola.__init__(self, c0, c1, c2)
		self.c3 = c3
	def __call__(self, x):
		print 'Cubic'
		return Parabola.__call__(self, x) + self.c3*x**3

class Poly4(Cubic): #inheriting from Cubic
	def __init__(self, c0, c1, c2, c3, c4):
		Cubic.__init__(self, c0, c1, c2, c3) 
		self.c4 = c4
	def __call__(self, x):
		print 'Poly4'
		return Cubic.__call__(self, x) + self.c4*x**4

y = Cubic(1,2,3,4) #creating instances
x = Poly4(1,2,3,4,5)
print x(3)
print y(3)

"""
terminal>> python Cubic_Poly4.py 
Poly4
Cubic
Parabola
Line
547
Cubic
Parabola
Line
142

"""
